...
В меню Управления интерфейсного окна пункт 'Записать/обновить скрипты БД' выполняет модуль ```db_scripts_create.py```. Модуль создает две хранимые процедуры в БД - выгрузки сырых данных в заданном диапазоне и выгрузки отчетных данных.

Остальной функционал реализован в основном поле интерфейсного окна.

Для работы, помимо встроенных в Python библиотек, используются дополнительные библиотеки:
...
* ```db_upload_module.py``` - модуль чтения Excel binary файла условленного формата и загрузки прочитанных данных в БД
